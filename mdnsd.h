 ...
 * Global functions
 *
 * create a new mdns daemon for the given class of names (usually 1) and max
 * frame size
 */
...
/*
 * outgoing messge to be delivered to host, returns >0 if 1 was returned & m/ip/port set
 */
 ...
 *   answer(record, arg) is called whenever 1 is found/changes/expires
 *   (immediate or anytime after, mdnsda valid until ->ttl==0)
 *   either answer returns -1, or another mdnsd_query with a NULL answer will
 ...
 *
 * create a new unique record (try mdnsda_list 1st to make sure it is not used)
 *   conflict(arg) called at any point when 1 is detected and unable to recover
 *   after the first data is set_*(), any future changes effectively expire the
 *   old one and attempt to create a new unique record
 */
...
/*
 * these all set/update the data for the given record, nothing is published
